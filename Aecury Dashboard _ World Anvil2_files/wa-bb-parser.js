 ...
 * @author Sam Clarke
 * 
 * @modifiedBy Stephan Franz (Xuroth)
 */
(function (sceditor) {
	...
	var defaultCommandsOverrides = {
		...
		image: {
			txtExec: function (caller, selected) {
				...
				getEditorCommand('image')._dropDown(
					...
					caller,
					selected,
					function (id, align, width, nolink) {
						var attrs = ':' + id;
						if (align && align !== '') {
							attrs += '|' + align;
							if (width) {
		...
		// 		this.insertText('[h1]' + selected + '[/h1]')
		// 	}
		// }
	};
	...
	var bbcodeHandlers = {
		...
		ul: {
			...
			breakAfter: true,
			// validChildren: ['li','ul','ol'],
			// breakBefore: true,
			format: '[ul]\n{0}[/ul]',
			// html: '<ul>{0}</ul>'
			html: function (token, attrs, content) {
				// console.log('content: ', content.substring(0,6));
				if (content.substring(0, 6) === '<br />') {
		...
		url: {
			...
			format: function (element, content) {
				...
				return '[url:' + url + (newTab ? '|tab' : '') + ']' + content + '[/url]';
			},
			html: function (token, attrs, content) {
				attrs.defaultattr =
	...
	// var TOKEN_SOURCE = 'source';
	var TOKEN_OPEN = 'open';
	var TOKEN_CONTENT = 'content';
	var TOKEN_NEWLINE = 'newline';
	var TOKEN_CLOSE = 'close';
	var TOKEN_ARTICLE = 'article';
	var TOKEN_KEYVAL = 'keyval';
	...
	TokenizeToken.prototype = {
		 ...
		 *                         empty clone if invalid splitAt location
		 */
	...
	function BBCodeParser(options) {
		...
		base.tokenize = function (str) {
			...
			var tokenTypes = [
				...
				{
					type: TOKEN_CONTENT,
					// Regex provided by @CaptainFry, modified by @Xuroth, reimplemented with prefab negative lookahead by Mihir Gokani
					// regex: /^([^\[\r\n]*|\[)[^\@\[]/ //<- BUGGY!
					// regex: /^([^\[\r\n\@]*|\[)/ //<- Works, but doesn't catch --key::val--
					regex: /^(?:(?!\-\-|[\r\n\@]|\[)[\w\W\s\d])*/
				},
				{
					type: TOKEN_NEWLINE,
				...
				{
					type: TOKEN_ARTICLE,
					// regex: /^(\@\[[a-zA-Z0-9\s\-\_\?\!\:\(\)]+\]\([a-zA-Z0-9\-\_\:]*\))/ //Legacy, only supports latic chars.
					// Regex provided by @CaptainFry
					regex: /^\@(\[?[\d\w\W?]*?\]?)\s?\(([\w\d:-]*)\)/ //New support for non-latin chars. Does not currently support "new article" template
				},
				{
					type: TOKEN_KEYVAL,
					regex: /^\-\-([\s\w\W\d]*?)\:\:([\s\w\W\d]*?)\-\-/
				}
			];
			...
			while (str.length) {
				...
				while (i--) {
					...
					if (str.match(/\u200c/ig)) {
						// console.log('Zero-Width space detected!')
						str = str.replace(/\u200c/, '')
					}

					// console.log('Matching Type: ' + type, str)
					if (str.match(tokenTypes[i].regex)) {
						// console.log('Type: '+type, 'Match: ' + str.match(tokenTypes[i].regex)[0])
					}
					...
					if (type === TOKEN_ARTICLE) {
						// console.log('A wild Article appeared!')
						// console.log(matches)

					}
					if (type === TOKEN_KEYVAL) {
						// console.log('KEYVAL detected!', matches,'\n' + str)

					}
					// Add the match to the tokens list and remove the match.
					...
					// console.log('First Character ', str.charCodeAt(0))
					tokens.push(tokenizeTag(type, matches[0]));
					str = str.substr(matches[0].length);
					// The token has been added so start again
					continue strloop;
				}
				// console.log('Remaining String',str)
		...
		function tokenizeTag(type, val) {
			var matches, attrs, name,
				...
				openRegex = /\[([^\]\s=|:]+)(?:([^\]]+))?\]|(\--[^\s=|:]+)/,
				closeRegex = /\[\/([^\[\]]+)\]/,
				articleRegex = /\@(\[?[\d\w\W?]*?\]?)\s?\(([\w\d:-]*)\)/,
				keyvalRegex = /\-\-([\s\w\W\d]*?)\:\:([\s\w\W\d]*?)\-\-/

			// console.log('val',val)
			// console.log('type',type)
			// console.log(val)
			if (type === TOKEN_KEYVAL && (matches = val.match(keyvalRegex))) {
				// console.log('KEYVAL Tokenization!');
				name = 'keyval'
				type = TOKEN_OPEN;
				attrs = tokenizeAttrs(':' + matches[1] + '|' + matches[2], name);
				tokenizeTag
			} else
				if (type === TOKEN_ARTICLE && (matches = val.match(articleRegex))) {
					// console.log('Article Match: ', matches)
					name = 'article'
					type = TOKEN_OPEN;
					attrs = tokenizeAttrs(':' + matches[1] + '|' + matches[2], name)
					// console.log(matches)
					//Extract any content around the article and parse it separately
					var contentBefore = matches.input.substring(0, matches.input.indexOf(matches[1]))
					var contentAfter = matches.input.substring(matches.input.indexOf(matches[2]) + matches[2].length)
					// console.log('content',contentBefore, contentAfter)
					tokenizeTag('content', contentBefore + contentAfter)
				} else
					...
					if (type === TOKEN_OPEN && (matches = val.match(openRegex))) {
						...
						if (matches[2] && (matches[2] = matches[2].trim())) {
							// Modified to pass the tag name
							// console.log('here')
							attrs = tokenizeAttrs(matches[2], name);
			...
			if (name === 'keyval') {
				// console.log('HERE')
				// console.log(type, name, val, attrs);
			}
			// console.log('tokenArgs',type, name, val, attrs);
		 ...
		 * @private
		 */
		function tokenizeAttrs(attrs, tagType = null) {
			// console.log('tokenizeAttrs',attrs)
			var matches,
				/*
				...
				*/
				// Modified to parse for ':' or '|' option denotation in tag
				attrRegex = /([^\s=]+)=(?:(?:(["'])((?:\\\2|[^\2])*?)\2)|((?:.(?!\s\S+=))*.))/g,
				ret = {};

			// if only one attribute then remove the = from the start and
			// strip any quotes
			if (attrs.charAt(0) === '=' && attrs.indexOf('=', 1) < 0 && tagType !== 'url') {
				ret.defaultattr = _stripQuotes(attrs.substr(1));
				// console.log('here')
				// [WA] Process custom WA-format tags
			} else if (attrs.charAt(0) === ":" || attrs.charAt(0) === '|' || attrs.indexOf('--') >= 0 || attrs.charAt(0) === '@' || attrs.charAt(0) === '=') {
				//Remove leading option character

				attrs = attrs.substring(1);
				// console.log('here')
				// console.log(attrs)
				// If a tagType is passed, handle it.
				if (tagType) {
					switch (tagType) {
						case 'h1':
						case 'h2':
						...
						case 'img': {
							var options = attrs.split('|');
							if (options[0]) {
								ret['data-wa-img-id'] = options[0];
							}
							if (options[1]) {
								ret['data-wa-img-align'] = options[1];
							}
							if (options[2]) {
								ret['data-wa-img-width'] = options[2];
							}
							if (options[3]) {
								ret['data-wa-img-nolink'] = options[3];
							}
							break;
						}
						case 'youtube': {
							ret['data-wa-youtube-id'] = attrs;
							// console.log(ret)
							break;
						}
						case 'spotify': {
							var options = attrs.split(':');
							// console.log('Spotify Attributes: ', attrs)
							// console.log('Options array: ', options)
							var type = options[0];
							if (type === 'http' || type === 'https') {
								// console.log('Type is URL', options);
								var url = options[1];
								// console.log(url)
								var id = url.match(/[^\/]*[0-9a-zA-Z]+$/)[0];
								var url = url.substring(0, url.length - id.length - 1);
								var param = url.match(/[^\/]*[0-9a-zA-Z]+$/)[0];
								url = url.substring(0, url.length - param.length - 1);
								// console.log('Data: ' + url,param,id);
								ret['data-wa-spotify-id'] = param + "/" + id;
								ret['data-wa-spotify-type'] = type;
								ret['data-wa-spotify-url-base'] = url + '/';
								// console.log(url)
								break;
							}
							if (type === 'spotify') {
								// console.log('Type is SPOTIFY', options)
								options.shift();
								ret['data-wa-spotify-id'] = options.join('/')
								ret['data-wa-spotify-type'] = options[0];
								break;
							}
							// ret['data-wa-spotify-type'] = options[0];
							// ret['data-wa-spotify-id'] = options[1];
							break;
						}
						case 'section': {
							ret['data-wa-section-id'] = attrs;
							break;
						}
						case 'container': {
							ret['data-wa-container-id'] = attrs;
							break;
						}
						case 'anchor': {
							ret['id'] = attrs;
							ret['data-wa-anchor-id'] = attrs;
							break;
						}
						case 'roll': {
							ret['data'] = attrs;
						...
						case 'redacted': {
							ret['data-width'] = attrs;
							break;
						}
						case 'color': {
							ret['data-color'] = attrs;
							break;
						}
						case 'podbean': {
							ret['data-wa-podbean-url'] = attrs;
							break;
						}
						case 'soundcloud': {
							ret['data-wa-soundcloud-url'] = attrs;
							break;
						}
						case 'kuula': {
							ret['data-wa-kuula-url'] = attrs;
							break;
						}
						case 'sketchfab': {
							ret['data-wa-sketchfab-url'] = attrs;
							break;
						}
						case 'anchorfm': {
							ret['data-wa-anchorfm-url'] = attrs;
							break;
						}
						case 'genesysdice': {
							ret['data-wa-genesysdice-type'] = attrs;
							break;
						}
						case 'calendar': {
							ret['data-calendar-id'] = attrs;
							break;
						}
						case 'book': {
							ret['data-wa-category-id'] = attrs;
							break;
						}
						case 'url': {
							// console.log('attrs', attrs)
							var attrArr = attrs.split('|');
							// console.log('attrs', attrArr)
							if (attrArr.length > 1) {
								ret['data-wa-url-link'] = attrArr[0];
								ret['data-wa-url-newtab'] = attrArr[1];
							} else {
								ret['data-wa-url-link'] = attrs;
							}
							break;
						}
						case 'keyval': {
							// console.log('keyval attrs', attrs);
							//Need to remove one more (one already done) '-'
							var params = attrs.split('|')
							// var key = attrs.substring(1, attrs.indexOf(':'));
							// var val = attrs.substring(attrs.indexOf(':')+2, attrs.length -2);
							ret['data-wa-keyval-key'] = params[0];
							ret['data-wa-keyval-val'] = params[1];
							// console.log(ret)
						...
						case 'article': {
							...
							ret['data-wa-article-name'] = attrs[0];
							ret['data-wa-article-link'] = attrs[1];
							// console.log(ret)
							break;
						}
						case 'map': {
							ret['data-wa-map-id'] = attrs;
							break;
						}
						case 'history': {
							ret['data-wa-event-id'] = attrs;
							break;
						}
						case 'category': {
							ret['data-wa-category-id'] = attrs;
						...
						case 'toc': {
							if (attrs.length > 0) {
								// console.log('toc attrs',attrs)
								ret['data-wa-toc-id'] = attrs;
							}

							break;
						}
						case 'secret': {
							ret['data-wa-secret-id'] = attrs;
							break;
						}
						case 'hero': {
							ret['data-wa-character-id'] = attrs;
							break;
						}
		...
		base.parse = function (str, preserveNewLines) {
			...
			// console.log(parseTokens(base.tokenize(str)))
			var opts = base.opts;
			...
			while ((token = toks.shift())) {
				...
				switch (token.type) {
					...
					case TOKEN_OPEN:
						...
						// console.log('bbcode Handler', bbcode)
		...
		function fixNesting(children, parents, insideInline, rootArr) {
			console.log("Attempting to fix Nesting")
		...
		function convertToHTML(tokens, isRoot) {
			var undef, token, bbcode, content, html, needsBlockWrap,
				...
				ret = [];
			// console.log('Tokens (tohtml)', JSON.stringify(tokens))
			isInline = function (bbcode) {
				return (!bbcode || (bbcode.isHtmlInline !== undef ?
					bbcode.isHtmlInline : bbcode.isInline)) !== false;
			};
			// console.log('Token ', token.name, ' is inline: ', isInline)

			while (tokens.length > 0) {
				if (!(token = tokens.shift())) {
					continue;
				}
				if (token.name === 'keyval') {
					// console.log(token)
				}
				if (token.name === 'quote') {
					// console.log('Quote is detected', token)
				}
				...
				if (token.type === TOKEN_OPEN) {
					...
					if (bbcode && bbcode.html) {
						...
						if (!isFunction(bbcode.html)) {
							...
							// html = bbcode.html(token, token.attrs, content)
						}
					} else {
					...
					if (!isRoot) {
						// console.log(token.name, 'is not root')
						ret.push('<br />');
						continue;
					}
			...
			for (var i = 0; i < ret.length; i++) {
				if (ret[i] === '<div>' && ret[i + 1] === '</div>') {
					// console.log('Found empty div.')
					ret[i] = '<br class="testBR">';
					ret[i + 1] = '';
				}
			}
		...
		base.toBBCode = function (str, preserveNewLines) {
			str = str.replace(/\u200c/ig, '')
			var toks = base.parse(str, preserveNewLines);
			// console.log(toks)
			return convertToBBCode(toks);
		};
		 ...
		 * @private
		 */
		function convertToBBCode(toks, lastToken = null) {
			// console.log(toks)
			...
			var lastTokenType = (lastToken ? lastToken.type : null);
			var nextToken = null;
			// console.log('convertToBBCode', toks[0])
			while (toks.length > 0) {
				if (!(token = toks.shift())) {
					continue;
				}
				...
				if (!bbcode && token.type === TOKEN_OPEN) {
					...
					if (token.children) {
						ret.push(convertToBBCode(token.children, token));
					}
					...
					if (breakBefore) {
						if (ret[ret.length - 1] === '\n') {
							// console.log('Previous token was newline: ',lastTokenType)
					...
					if (token.name === ('table' || 'tr' || 'th' || 'td')) {
						// console.log(ret)
					}
					// Convert the tag and it's attributes to BBCode (except for key::val)
					if (token.name !== 'key' && token.name !== 'article' && token.name !== 'keyval') {
						ret.push('[' + token.name);
					}
					if (token.attrs) {
						...
						// console.log('toBBCode', token)
						// [WA] Handle custom WA format tags
						switch (token.name) {
							// [h1|ID] - [h6|ID]
							case 'h1':
							case 'h2':
							...
							case 'h6': {
								if (token.attrs['id']) {
									ret.push('|', token.attrs['id'])
								}
								break;
							}
							case 'br': {
								// console.log('Custom Linebreak')
								break;
							}
							case 'article': {
								// console.log('TOKEN ATTRS', token)
								ret.push('@' + token.attrs['data-wa-article-name'])
								ret.push('(' + token.attrs['data-wa-article-link'] + ')')
								break;
							}
							// [IMG:ID|Align|Width|nolink]
							case 'img': {
								ret.push(':');
								if (token.attrs['data-wa-img-id']) {
									ret.push(token.attrs['data-wa-img-id']);
								}
								if (token.attrs['data-wa-img-align']) {
									ret.push('|', token.attrs['data-wa-img-align']);
								}
								if (token.attrs['data-wa-img-width']) {
									ret.push('|', token.attrs['data-wa-img-width']);
								}
								if (token.attrs['data-wa-img-nolink']) {
									ret.push('|', token.attrs['data-wa-img-nolink'])
								}
								break;
							}
							...
							case 'youtube': {
								ret.push(':');
								ret.push(token.attrs['data-wa-youtube-id'])
								break;
							}

							case 'spotify': {
								// console.log('here',token.attrs)
								var type = token.attrs['data-wa-spotify-type'];
								if (type === 'http' || type === 'https') {
									ret.push(':' + type + ':' + token.attrs['data-wa-spotify-url-base'] + token.attrs['data-wa-spotify-id']);
									break;

								} else {
									ret.push(':spotify:' + token.attrs['data-wa-spotify-id'].split('/').join(':'))
								}
							...
							case 'section': {
								ret.push(':' + token.attrs['data-wa-section-id'])
								break;
							}

							case 'container': {
								ret.push(':' + token.attrs['data-wa-container-id'])
								break;
							}
							case 'anchor': {
								ret.push('|' + token.attrs['data-wa-anchor-id'])
								break;
							}
							case 'roll': {
								ret.push(':' + token.attrs['data'])
								break;
							}
							case 'redacted': {
								ret.push(':' + token.attrs['data-width']);
								break;
							}
							case 'color': {
								ret.push(':' + token.attrs['data-color']);
								break;
							}
							case 'podbean': {
								ret.push(':' + token.attrs['data-wa-podbean-url']);
								break;
							}
							case 'soundcloud': {
								ret.push(':' + token.attrs['data-wa-soundcloud-url']);
								break;
							}
							case 'kuula': {
								ret.push(':' + token.attrs['data-wa-kuula-url']);
								break;
							}
							case 'sketchfab': {
								ret.push(':' + token.attrs['data-wa-sketchfab-url']);
								break;
							}
							case 'anchorfm': {
								ret.push(':' + token.attrs['data-wa-anchorfm-url']);
								break;
							}
							case 'genesysdice': {
								ret.push(':' + token.attrs['data-wa-genesysdice-type']);
								break;
							}
							case 'calendar': {
								ret.push(':' + token.attrs['data-calendar-id']);
								break;
							}
							case 'book': {
								ret.push(':' + token.attrs['data-wa-category-id']);
								break;
							}
							case 'keyval': {
								// console.log('ConvertToBBCode key function')
								ret.push('--' + token.attrs['data-wa-keyval-key'] + '::');
								ret.push(token.attrs['data-wa-keyval-val'] + '--');
								break;
							}
							case 'map': {
								ret.push(':', token.attrs['data-wa-map-id']);
								break;
							}
							case 'history': {
								ret.push(':' + token.attrs['data-wa-event-id']);
								break;
							}
							case 'category': {
								ret.push(':' + token.attrs['data-wa-category-id']);
							...
							case 'toc': {
								if (token.attrs['data-wa-toc-id']) {
									// console.log('toc-id')
									ret.push(':' + token.attrs['data-wa-toc-id']);
								}

								break;
							}
							case 'secret': {
								ret.push(':' + token.attrs['data-wa-secret-id']);
								break;
							}
							case 'hero': {
								ret.push(':' + token.attrs['data-wa-character-id']);
								break;
							}
							case 'url': {
								// console.log('token',token)
								ret.push(':' + token.attrs['data-wa-url-link']);
								if (token.attrs['data-wa-url-newtab']) {
									ret.push('|tab')
								}
								break;
							}
					...
					if (token.name !== 'keyval' && token.name !== 'article') {
						ret.push(']');
					}


					if (breakStart) {
						// console.log(token.name, "breakStart")
						if (ret[ret.length - 1] !== '\n') {
							ret.push('\n');
						}
						// console.log(ret)
					...
					if (token.children) {
						ret.push(convertToBBCode(token.children, lastToken));
					}
					...
					if (breakAfter) {
						if (ret[ret.length - 1] === '\n') {
							// console.log('Previous element is a linebreak')
						}
						if (ret[ret.length - 1] !== '\n') {
							ret.push('\n');
						}
			...
			var bbstring = ret.join('');

			// console.log('Before: ',JSON.stringify(bbstring));
			// bbstring = bbstring.replace(/[\n]{2,}/g, '\n')
			// console.log('After: ', JSON.stringify(bbstring))
	...
	function bbcodeFormat() {
		...
		function toSource(asFragment, html, context, parent) {
			...
			// console.log('bbcode before',bbcode)
			bbcode = parser.toBBCode(bbcode, true);
			// console.log('bbcode after',bbcode)
